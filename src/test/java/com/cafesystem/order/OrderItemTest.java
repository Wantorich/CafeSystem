...
public class OrderItemTest {
  ...
  void 가격이_4500원인_메뉴를_3개_주문하면_소계는_13500원이다() {
    SingleMenu singleMenu = SingleMenu.createMenu("카페라떼", Price.of(4500),  Category.COFFEE);
  ...
  void 음료에_크기를_포함해서_주문한다() {
    SingleMenu americano = DrinkMenu.create(DrinkType.AMERICANO, DrinkSize.MEDIUM);
  ...
  void 음료에_옵션_1개를_추가시_기본가격에_옵션가격이_포함된다() {
    SingleMenu cafeLatte = DrinkMenu.create(DrinkType.CAFE_LATTE, DrinkSize.LARGE);
  ...
  void 음료에_옵션_2개를_추가할시_기본가격에_옵션_합산가격이_포함된다() {
    SingleMenu americano = DrinkMenu.create(DrinkType.AMERICANO, DrinkSize.SMALL);
  ...
  void 음료를_옵션없이_주문하면_기본가격으로만_계산된다() {
    SingleMenu cafeLatte = DrinkMenu.create(DrinkType.CAFE_LATTE, DrinkSize.SMALL);
